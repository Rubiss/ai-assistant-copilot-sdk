...
export interface RestartConfig {
  /** Services allowed to restart (use "*" for all) */
  allowlist: string[];
  /** Min time between restarts per service (default: 5 min) */
  cooldownMs: number;
  /** Whether to require operator approval */
...
export async function restartService(
  ...
  // Pre-check: allowlist
  if (
    !config.allowlist.includes(containerName) &&
  ...
  if (lastRestart && Date.now() - lastRestart < config.cooldownMs) {
    const remaining = Math.ceil(
      (config.cooldownMs - (Date.now() - lastRestart)) / 1000,
    );
    return {
      success: false,
