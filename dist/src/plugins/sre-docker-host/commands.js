...
export const incidentCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) => sub.setName("list").setDescription("List open incidents"))
    ...
    execute: async (interaction) => {
        ...
        switch (sub) {
            case "list": {
                ...
                const lines = all.map((i) => `• **${i.severity?.toUpperCase() ?? "?"}** \`${i.id.slice(0, 8)}\` ${i.title} (${i.status})`);
            ...
            case "ack": {
                ...
                await interaction.reply(`✅ Incident \`${id.slice(0, 8)}\` acknowledged.`);
                break;
            }
            case "note": {
                const id = interaction.options.getString("id", true);
                ...
                await interaction.reply(`📝 Note added to incident \`${id.slice(0, 8)}\`.`);
                break;
            }
        }
    },
...
export const reportNow = {
    data: new SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: "Daily Health", value: "daily" }, { name: "Weekly Summary", value: "weekly" })))
        .toJSON(),
    execute: async (interaction) => {
        await interaction.deferReply();
        // Will integrate with report generation in Phase 6.9
        await interaction.editReply("📊 Report generation queued. Results will appear shortly.");
    },
};
